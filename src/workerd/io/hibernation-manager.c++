    ...
    KJ_IF_MAYBE(error, maybeError) {
      ...
      if (!refToHibernatable.hasDispatchedClose &&
        ...
        event = workerInterface->customEvent(kj::heap<api::HibernatableWebSocketCustomEventImpl>(
            ...
            .then([&](auto _) {
              refToHibernatable.hasDispatchedClose = true;
              auto& ws = KJ_REQUIRE_NONNULL(refToHibernatable.activeWebSocket);
              ws->closeIncoming();
              ws->tryReleaseNative();
        ...
        event = workerInterface->customEvent(kj::heap<api::HibernatableWebSocketCustomEventImpl>(
            ...
            .then([&](auto _) {
              auto& ws = KJ_REQUIRE_NONNULL(refToHibernatable.activeWebSocket);
              ws->closeIncoming();
              // TODO(now): We need to do some cleanup similar to `reportError()`.
            }).eagerlyEvaluate(nullptr);
      }
    }
  ...
  return ws.receive()
    ...
    return workerInterface->customEvent(kj::heap<api::HibernatableWebSocketCustomEventImpl>(
        ...
        .then([this, &hib, isClose=isClose](auto _) -> kj::Promise<void> {
          if (isClose) {
            // Release now that we've closed.
            KJ_REQUIRE_NONNULL(hib.activeWebSocket)->tryReleaseNative();
            return kj::READY_NOW;
          }
          return readLoop(hib);
        }).eagerlyEvaluate(nullptr);
